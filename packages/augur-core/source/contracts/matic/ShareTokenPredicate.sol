...
contract ShareTokenPredicate is Initializable {
     ...
     */
    function parseData(bytes calldata data) external view returns(address account, address market, uint256 outcome, uint256 balance, uint256 age) {
        ...
        age = withdrawManager.verifyInclusion(data, 0 /* offset */, false /* verifyTxInclusion */);
    ...
    function executeInFlightTransaction(bytes memory txData, address signer, IShareToken exitShareToken) public returns(int256 nonce) {
        ...
        if (funcSig == UNSAFE_TRANSFER_FROM_FUNC_SIG) {
            ...
            exitShareToken.unsafeTransferFrom(from, to, repackageTokenId(id), value);
        } else if (funcSig == UNSAFE_BATCH_TRANSFER_FROM_FUNC_SIG) {
            (address from, address to, uint256[] memory id, uint256[] memory value) = abi.decode(
                BytesLib.slice(txData, 4, txData.length - 4),
                (address, address, uint256[], uint256[])
            );
            require(signer == from, "signer != from");
            exitShareToken.unsafeBatchTransferFrom(from, to, repackageTokenIds(id), value);
        } else {
    ...
    function repackageTokenId(uint256 _tokenId) internal view returns (uint256 /* tokenId */) {
        (address _market, uint256 _outcome) = unpackTokenId(_tokenId);
        (address _rootMarket,,) = predicateRegistry.childToRootMarket(_market);
        require(
