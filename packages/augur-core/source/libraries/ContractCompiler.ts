...
export class ContractCompiler {
    ...
    public async generateFlattenedSolidity(filePath: string): Promise<string> {
        ...
        if (['IExchange', 'FillOrder', 'ZeroXTrade', 'ZeroXExchange', 'SimulateTrade', 'IZeroXTrade', 'ZeroXTradeToken', 'AugurPredicate'].includes(path.parse(filePath).base.replace(".sol", ""))) {
