...
contract ExitShareToken is ITyped, ERC1155, IExitShareToken, ReentrancyGuard, ExecutorAcl {
    ...
    */
     ...
     * @return (uint256 _creatorFee, uint256 _reportingFee) The fees taken for the market creator and reporting respectively
     */
    function sellCompleteSets(IMarket _market, address _holder, address _recipient, uint256 _amount, bytes32 _fingerprint) external isExecuting returns (uint256 _creatorFee, uint256 _reportingFee) {
     ...
     * @return Bool True
     */
    function claimTradingProceeds(IMarket _market, address _shareHolder, bytes32 _fingerprint) external isExecuting nonReentrant returns (uint256[] memory _outcomeFees) {
    ...
    function mint(address to, IMarket market, uint256 outcome, uint256 balance) external isExecuting {
        _mint(to, TokenId.getTokenId(market, outcome), balance, bytes(""), false);
    }
}
